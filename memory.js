window.onload = start;

function start()
{
var cards = ["ciri.png", "geralt.png", "jaskier.png", "jaskier.png",
                     "iorweth.png", "triss.png", "geralt.png", "yen.png",
			   "ciri.png", "triss.png", "yen.png", "iorweth.png"];
			   
	$('.board').html('<div class="card" id="c0" ></div>'+
					'<div class="card" id="c1" ></div>'+
					'<div class="card" id="c2"></div>'+
					'<div class="card" id="c3"></div>'+
					
					'<div class="card" id="c4"></div>'+
					'<div class="card" id="c5"></div>'+
					'<div class="card" id="c6"></div>'+
					'<div class="card" id="c7"></div>'+
					
					'<div class="card" id="c8"></div>'+
					'<div class="card" id="c9"></div>'+
					'<div class="card" id="c10"></div>'+
					'<div class="card" id="c11"></div>'+
					
					'<div class="score">Turn counter: 0</div>');
			   
//alert(cards[4]);

//console.log(cards);

var c0 = document.getElementById("c0");
var c1 = document.getElementById("c1");
var c2 = document.getElementById("c2");
var c3 = document.getElementById("c3");

var c4 = document.getElementById("c4");
var c5 = document.getElementById("c5");
var c6 = document.getElementById("c6");
var c7 = document.getElementById("c7");

var c8 = document.getElementById("c8");
var c9 = document.getElementById("c9");
var c10 = document.getElementById("c10");
var c11 = document.getElementById("c11");

c0.addEventListener("click", function()  { revealCard(0); });
c1.addEventListener("click", function()  { revealCard(1); });
c2.addEventListener("click", function()  { revealCard(2); });
c3.addEventListener("click", function()  { revealCard(3); });

c4.addEventListener("click", function()  { revealCard(4); });
c5.addEventListener("click", function()  { revealCard(5); });
c6.addEventListener("click", function()  { revealCard(6); });
c7.addEventListener("click", function()  { revealCard(7); });

c8.addEventListener("click", function()  { revealCard(8); });
c9.addEventListener("click", function()  { revealCard(9); });
c10.addEventListener("click", function()  { revealCard(10); });
c11.addEventListener("click", function()  { revealCard(11); });

var oneVisible = false;
var turnCounter = 0;
var cardInMemo;
var lockPair = false;
var pairsLeft = 6;

function revealCard(nr)
{
	var opacityValue = $('#c'+nr).css('opacity');
	
	//alert('Opacity: ' + opacityValue);  //Pobieranie wartości, działa :)
	
	if(opacityValue != 0 && lockPair == false)
	{
		lockPair = true;
		
		//alert(nr);
			
		var picture = "url(img/" + cards[nr] + ")";
			
		$('#c'+nr).css('background-image', picture);
		$('#c'+nr).addClass('cardA');
		$('#c'+nr).removeClass('card');
			
		if(oneVisible == false) 
		{
			//first card
				
			oneVisible = true;
			cardInMemo = nr;
			lockPair = false;
		}
		else
		{
			//second card
				
			if(cards[cardInMemo] == cards[nr])
			{
				//alert("hit");
					
				setTimeout(function() { hidePair(nr, cardInMemo) },  750);
			}
			else
			{
				//alert("mishit");
				
				setTimeout(function() { restorePair(nr, cardInMemo) },  1000);
			}
				
			turnCounter++;
			$('.score').html('Turn counter: ' + turnCounter);
			oneVisible = false;
		}
	}
}

function hidePair(nr1, nr2)
{
	$('#c'+nr1).css('opacity', '0');
	$('#c'+nr2).css('opacity', '0');
	
	pairsLeft--;
	
	if(pairsLeft == 0)
	{
		$('.board').html('<h1 style="padding-top: 85px;">You win!<br>Done in '+ turnCounter +' turns</h1><br><div class="buttonRepeat" onclick="location.reload()">Repeat?</div>');
	}
	
	lockPair = false;
}

function restorePair(nr1, nr2)
{
	$('#c'+nr1).css('background-image', 'url(img/card.png)');
	$('#c'+nr1).addClass('card');
	$('#c'+nr1).removeClass('cardA');
	
	$('#c'+nr2).css('background-image', 'url(img/card.png)');
	$('#c'+nr2).addClass('card');
	$('#c'+nr2).removeClass('cardA');
	
	lockPair = false;
}
}